...
public:
    ...
    void nhapDanhSach(){
        ...
        if (MaSach.size() > 30) {
            cout << "Không đủ bộ nhớ để lưu các đầu sách. Tối đa 30, vui lòng xoá bớt nếu bạn thêm vào." << endl;
        } else {
            for (int i = 0; i < n; i++) {
    ...
    void xuatDanhSach(){
       ...
       for (int i = 0; i < MaSach.size(); i++) {
            ...
            << " | Đơn giá: " << DonGia[i] << " | Số lượng: " << SL[i] << " | Tác giả: " << TenTG[i] << " | Nhà xuất bản: " << NXB[i] << endl;
            cout << "============================================" << endl;
       }
    }
    ...
    void sapXepDauSachTheoSoLuong(){
        ...
        for (int i = 0; i < MaSach.size() - 1; i++) { 
            for (int j = 0; j < MaSach.size() - i - 1; j++) {                            
                if (SL[j] > SL[j + 1]){       
                    swapstring(MaSach[j], MaSach[j + 1]);
                    swapstring(TenSach[j], TenSach[j + 1]);
                    swapstring(NXB[j], NXB[j + 1]);
                    swapstring(TenTG[j], TenTG[j + 1]);
                    swapint(SL[j], SL[j + 1]);
